...
public class MergedFlow implements FlowDefinition {

    @Override
    public InitialNode getDefinition() {
        Merge merge = new Merge();
        merge.addOutgoingNode(new FinalNode());

        InitialNode node = new InitialNode();
        node.addOutgoingNode(
                new Fork().addOutgoingNode(new Task1().addOutgoingNode(merge))
                        .addOutgoingNode(new Task2().addOutgoingNode(merge)));
        return node;
    }
}
